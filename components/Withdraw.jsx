...
export default function Withdraw({ setTab, userId, balance }) {
  ...
  const [walletAddress, setWalletAddress] = useState(""); // 用于保存和显示用户的钱包地址
  const [newAddress, setNewAddress] = useState(""); // 新输入的钱包地址
  ...
  useEffect(() => {
    const fetchWalletAddress = async () => {
      ...
      if (error) {
        ...
        setWalletAddress(data.wallet_address); // 设置钱包地址
      }
    };

    if (userId) {
      fetchWalletAddress(); // 如果有用户 ID，获取钱包地址
    }
  }, [userId]);
