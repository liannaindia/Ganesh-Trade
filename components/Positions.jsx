  ...
  return (
    <div className="px-4 pb-24 max-w-md mx-auto">
      <div className="mt-3 mb-3 text-center">
        <h2 className="text-lg font-bold text-slate-800 border-b-2 border-yellow-400 inline-block pb-1">Positions</h2>
      </div>
      ...
      <div className="flex items-center border-b border-slate-200 mb-3">
        <button
          ...
          className={`flex-1 text-center py-2 text-sm font-semibold border-b-2 transition ${tab === "pending" ? "text-yellow-500 border-yellow-500" : "text-slate-500 border-transparent"}`}
        >
          Pending Order
        </button>
        <button
          onClick={() => setTab("completed")}
          className={`flex-1 text-center py-2 text-sm font-semibold border-b-2 transition ${tab === "completed" ? "text-yellow-500 border-yellow-500" : "text-slate-500 border-transparent"}`}
        >
          Completed
        </button>
      </div>
      ...
      <div className="space-y-3 max-h-[500px] overflow-y-auto">
        {list.map((o) => (
          <div key={o.id} className="bg-white border border-slate-200 rounded-2xl p-3 shadow-sm">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <img src={o.img} alt={o.name} className="w-10 h-10 rounded-full object-cover" />
                <div>
                  <div className="font-semibold text-slate-800 text-sm">{o.name}</div>
                  <div className="text-[12px] text-slate-500">Investment Experience {o.years} years</div>
                </div>
              </div>
            ...
            <div className="grid grid-cols-2 mt-2 text-[12px] text-slate-500">
              ...
              <div className="col-span-2 flex justify-between mt-2 text-[12px]">
                ...
                <div className="text-right">
                  Order status <br />
                  <span className={`font-semibold ${o.status === "Following" ? "text-yellow-500" : "text-emerald-600"}`}>
                    {o.status}
                  </span>
                </div>
              </div>
